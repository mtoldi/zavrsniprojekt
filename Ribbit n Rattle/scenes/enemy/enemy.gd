...
func _ready() -> void:
	...
	if not status_handler._has_status("thirsty"):
		Events.thirsty_expired.emit()
		
	if anim:
		anim.play("idle")
		
		if anim.sprite_frames.has_animation("spell"):
			Events.spell_casted.connect(spellcasters)
		
func spellcasters() -> void:
	anim.play("spell")
